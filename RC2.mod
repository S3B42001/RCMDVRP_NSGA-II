...
param dm{N} default 0; # demanda de un cliente
param d{i in N, j in N} >= 0 default 0; # Distancia entre nodos
param habilitado{o in O, k in K} default 0;
...
param M1 := sum{i in R} dm[i];          # Eli: Este es computable
param M2 default 1000000;                 # Valor grande
...
var Ri{i in N, k in K} >= 0;  # riesgo acumulado al salir del nodo i
var D{i in N, k in K} >= 0;   # valor acumulado al salir del nodo i
...
R1 {o in O, k in K}:
    sum {j in R} x[o,j,k] <= habilitado[o,k] ; #cambiar por habilitado.
...
R10{i in R union O, j in R union S, k in K: (i,j) in A}:
    Ri[j,k] >= Ri[i,k] + D[j,k]*d[i,j] - (1-x[i,j,k]) * M2 ;
...
R13{i in R union O, j in R union S, k in K: (i,j) in A}:
    D[j,k] >= D[i,k] + dm[j] - (1-x[i,j,k]) * M1 ;
